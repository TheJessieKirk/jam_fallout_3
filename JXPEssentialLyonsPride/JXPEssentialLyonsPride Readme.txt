                      JESSIE's ESSENTIAL LYONS' PRIDE MOD
                              1.1.2, January 2025

                                   1. LEGAL

Copyright © 2021, 2022, 2025 Jessie Kirk [http://jessiekirk.xyz/]

This software is free software: you can redistribute it and/or modify it under
the terms of the WTFPL, Version 2, as published by Sam Hocevar.

This software is distributed in the hope that it will be useful, but WITHOUT
ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
FOR A PARTICULAR PURPOSE.

You should have received a copy of the WTFPL along with this software. If not,
see [http://www.wtfpl.net/about/].

                         2. INSTALLATION INSTRUCTIONS

    1. Review the “JXPEssentialLyonsPride License.txt” file to ensure you
       understand your rights.

    2. Extract the file “JXPEssentialLyonsPride.esp” from the archive file
       “JXPEssentialLyonsPride.zip” to your PC's Fallout 3 Data directory. The
       usual locations of this directory are:

           Steam Version, 64-bit Windows:
             C:\Program Files (x86)\Steam\steamapps\common\Fallout 3 goty\Data

           Steam Version, 32-bit Windows:
             C:\Program Files\Steam\steamapps\common\Fallout 3 goty\Data

           Retail Version, 64-bit Windows:
             C:\Program Files (x86)\Bethesda Softworks\Fallout 3\Data

           Retail Version, 32-bit Windows:
             C:\Program Files\Bethesda Softworks\Fallout 3\Data

    3. Optionally, extract the files “JXPEssentialLyonsPride Readme.txt” and
       “JXPEssentialLyonsPride License.txt” to your PC's Fallout 3 Data
       directory to retain them for future reference.

    4. Launch Fallout 3. When the Launcher window appears, select “Data Files”.
       The Fallout 3 Data Files window will appear containing a list of files.
       Click/tap the checkbox next to “JXPEssentialLyonsPride.esp” to enable
       it, then click/tap the “OK” button. The mod is now installed.

                                3. GAME CHANGES

    This mod makes all the members of Lyons' Pride essential (unkillable),
    except the late Jennings and DOA Reddin.

    NOTE: This mod used to be called Jessie's Essential Pride Mod.

                                 4. RATIONALE

    In Broken Steel, Elder Lyons states that the Pride has been working to wipe
    out all remaining Enclave bastions. It's a bit hard to do that if a chunk
    of the squad got themselves killed.

                        5. UNINSTALLATION INSTRUCTIONS

    1. Delete the files “JXPEssentialLyonsPride.esp”. The mod is now
       uninstalled.

                                 6. CHANGELOG

    0.1.0
      Initial commit.
      Added "essential" flag to Knight Captain Dusk, Paladin Glade and Paladin
      Kodiak NPCs.

    1.0.0
      Updated file naming to new conventions.

    1.1.0
      Switched to script injection for better mod compatibility.

    1.1.1
      Fixed syntax errors in documentation.

    1.1.2
      Changed mod name to Essential Lyons' Pride.
      Commented script.
      REALLY fixed syntax errors in documentation.

                                  7. CREDITS

    xEdit used to clean mod file.
    [https://tes5edit.github.io/]

    GECK used to create mod file.
    [https://geck.bethsoft.com/index.php?title=Garden_of_Eden_Creation_Kit]

    Monofonto font by Raymond Larabie. Free font.
    [http://typodermicfonts.com/]

    Overseer Oblique font by Neale Davidson. Used under Pixel Saga's Font
    License.
    [http://www.pixelsagas.com/]
